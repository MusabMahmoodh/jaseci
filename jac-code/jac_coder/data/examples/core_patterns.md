...
```jac
"""Complete CRUD backend using nodes and typed returns."""
...
def:pub add_product(name: str, price: float, category: str = "") -> Product {
    return (root() ++> Product(name=name, price=price, category=category))[0];
}

def:pub get_products(category: str = "") -> list[Product] {
