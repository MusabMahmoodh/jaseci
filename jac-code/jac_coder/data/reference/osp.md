...
# Use jid(session) to get the unique graph identity — no manual id field needed
...
# Hierarchy: root → Category → Item (typed returns)
def:pub add_category(name: str) -> Category {
    return (root() ++> Category(name=name))[0];
}

def:pub add_item(category_name: str, title: str) -> Item {
    for cat in [root()-->][?:Category] {
        if cat.name == category_name {
            return (cat ++> Item(title=title))[0];
        }
    }
}

def:pub get_items_by_category(category_name: str) -> list[Item] {
    for cat in [root()-->][?:Category] {
        if cat.name == category_name {
            return [cat-->][?:Item];
        }
