...
# Typed returns — nodes auto-serialized, client gets dot access + jid()
def:priv get_todos -> list[Todo] {
    return [root()-->][?:Todo];
}
...
def:priv toggle_todo(todo_id: str) -> Todo {
    for t in [root()-->][?:Todo] {
        if jid(t) == todo_id {
...
def:priv delete_todo(todo_id: str) -> dict {
    for t in [root()-->][?:Todo] {
        if jid(t) == todo_id {
            del t;
            return {"deleted": True};
...
import from "@jac/runtime" { Router, Routes, Route, Navigate, jacIsLoggedIn }

# AuthGuard is currently broken — inline the check at the top of protected pages.
...
def:pub useTodos() -> dict {
    ...
    async def fetch_initial() -> None {
        try {
            todos = await get_todos() or [];   # list[Todo] typed objects
        } except Exception as e {
            msg = str(e);
    ...
    async def handleAdd(title: str) -> None {
        if not title { return; }
        try {
            todo = await add_todo(title);       # returns Todo typed object
            todos = todos + [todo];
        } except Exception as e {
            msg = str(e);
            if "UNAUTHORIZED" in msg { handleAuthError(); }
            else { error = msg; }
        }
    }

    async def handleToggle(todo_id: str) -> None {
        try {
            updated = await toggle_todo(todo_id);  # returns updated Todo
            todos = [
                (updated if jid(t) == todo_id else t)
                for t in todos
            ];
        } except Exception as e {
            msg = str(e);
    ...
    async def handleDelete(todo_id: str) -> None {
        try {
            await delete_todo(todo_id);
            todos = [t for t in todos if jid(t) != todo_id];
        } except Exception as e {
            msg = str(e);
            if "UNAUTHORIZED" in msg { handleAuthError(); }
            else { error = msg; }
...
def:pub TodoItem(props: dict) -> JsxElement {
    ...
    # Props are always dict — extract values with guards
    tid = props["todoId"] or "";
...
def:pub TodoList() -> JsxElement {
    ...
    total = len(todos);
    completed_count = len(
        [t for t in todos if t and t.completed]    # dot access on typed Todo
    );
...
| Pattern | Code |
|---|---|
| Register server function | `def:priv fn(arg: str) -> Todo { ... }` in a `.jac` file |
| Typed return | `def:pub get_todos -> list[Todo] { return [root()-->][?:Todo]; }` |
| Import server fn to client | `sv import from ..service.todoService { fn }` — must point at the defining module |
| Call server fn | `result = await fn(arg)` — returns typed object, use dot access |
...
| Get node ID | `jid(todo)` — unique graph identity, no manual uuid needed |
| Protected route (manual) | `if not jacIsLoggedIn() { return <Navigate to="/login" />; }` at top of page |
| Conditional JSX | `{condition and (<Element />)}` |
| List JSX (typed) | `{[<El key={jid(t)} title={t.title} /> for t in items]}` |
| List JSX (dict) | `{[<El key={t["id"]} /> for t in items]}` |
